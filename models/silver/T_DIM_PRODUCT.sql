...
WITH SOURCE_DATA AS (
    ...
    FROM {{ source('bronze_data', 'T_BRZ_ITEM_MASTER_INITEM') }} src
    {% if is_incremental() %}
        WHERE src.ENTRY_TIMESTAMP > (SELECT COALESCE(MAX(EFFECTIVE_DATE), '1899-12-31T00:00:00Z') FROM {{ this }})
    {% endif %}
    QUALIFY ROW_NUMBER() OVER (PARTITION BY CAST(TRIM(I0ITM) AS INTEGER) ORDER BY TO_TIMESTAMP_NTZ(TRIM(ENTRY_TIMESTAMP)) DESC) = 1
...
JOINED_DATA_WITH_CHECKSUM AS (
    ...
    SELECT
        ...
        MD5(CONCAT_WS('|',
            COALESCE(sd.ITEM_DESCRIPTION, ''),
            COALESCE(CAST(sd.CLASS_CODE AS VARCHAR), ''),
            COALESCE(sd.PRODUCT_CODE, ''),
            COALESCE(sd.SIZE, ''),
            COALESCE(sd.PRODUCT_SIZE, ''),
            COALESCE(sd.MANUFACTURER_CODE, ''),
            COALESCE(CAST(sd.WEIGHT AS VARCHAR), ''),
            COALESCE(CAST(sd.FEDERAL_TAX AS VARCHAR), ''),
            COALESCE(CAST(dim.INVENTORY_VENDOR_SK AS VARCHAR), ''),
            COALESCE(CAST(DC.INVENTORY_CLASS_SK AS VARCHAR), ''),
            COALESCE(CAST(DCAT.INVENTORY_CATEGORY_SK AS VARCHAR), '')
        )) AS RECORD_CHECKSUM_HASH

    FROM SOURCE_DATA sd
    LEFT JOIN {{ ref('T_DIM_INVENTORY_CLASS') }} DC
      ON CAST(TRIM(sd.CLASS_CODE) AS INTEGER) = dc.CLASS_ID
    LEFT JOIN {{ ref('T_DIM_INVENTORY_CATEGORY') }} DCAT
      ON CAST(TRIM(sd.CLASS_CODE) AS INTEGER) = DCAT.CATEGORY_ID
    LEFT JOIN {{ ref('T_DIM_INVENTORY_VENDOR') }} dim
      ON dim.VENDOR_NUMBER = CAST(TRIM(sd.INVENTORY_VENDOR_NUMBER) AS INTEGER)
     AND TO_TIMESTAMP_NTZ(TRIM(sd.ENTRY_TIMESTAMP)) BETWEEN dim.EFFECTIVE_DATE AND COALESCE(dim.EXPIRATION_DATE, '9999-12-31')
...
deletes AS (
    ...
    SELECT *
    FROM JOINED_DATA_WITH_CHECKSUM
...
EXPIRED_ROWS AS (
    ...
    SELECT
       ...
       EXISTING.EFFECTIVE_DATE,
        NEW_ROWS.EFFECTIVE_DATE - INTERVAL '1 second' AS EXPIRATION_DATE,
