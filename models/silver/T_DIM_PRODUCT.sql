...
WITH SOURCE_DATA AS (
    ...
    SELECT
        ...
        -- Updated MD5 hash to include dimension keys
        MD5(CONCAT_WS('|',
    ...
    LEFT JOIN {{ ref('T_DIM_INVENTORY_CLASS') }} DC
      ON CAST(TRIM(src.I0CLCD) AS INTEGER) = dc.CLASS_ID
    LEFT JOIN {{ ref('T_DIM_INVENTORY_CATEGORY') }} DCAT
      ON CAST(TRIM(src.I0CTCD) AS INTEGER) = DCAT.CATEGORY_ID
    LEFT JOIN {{ ref('T_DIM_INVENTORY_VENDOR') }} dim
      ON dim.VENDOR_NUMBER = CAST(TRIM(src.I0IVDN) AS INTEGER)
...
-- Simplified JOINED_CHANGES since joins are now in SOURCE_DATA
JOINED_CHANGES AS (
