...
source_data AS (
 SELECT
    ...
    ic.INVENTORY_CATEGORY_SK AS INVENTORY_CATEGORY_SK,
    ...
    LEFT JOIN  {{ source('silver_data', 'T_DIM_PRODUCT') }} dp
        ON CAST(TRIM(src.W2ITM) AS NUMBER(10, 0)) = dp.ITEM_NUMBER AND dp.IS_CURRENT_FLAG = TRUE
    LEFT JOIN {{ source('silver_data', 'T_DIM_MECHANIC') }} mech
        ON CAST(TRIM(src.W2MECH) AS NUMBER(10, 0)) = mech.MECHANIC_ID AND mech.IS_CURRENT_FLAG = TRUE 
    LEFT JOIN {{ source('silver_data', 'T_DIM_INVENTORY_CATEGORY') }} ic
        ON CAST(TRIM(src.W2CTCD) AS NUMBER(3, 0)) = ic.CATEGORY_ID AND ic.IS_CURRENT_FLAG = TRUE
    {% if is_incremental() %}
