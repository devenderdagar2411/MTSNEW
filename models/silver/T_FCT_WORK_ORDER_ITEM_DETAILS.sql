...
source_data AS (
 SELECT
    ...
    CAST(TRIM(W2ITM) AS NUMBER(10, 0)) AS PRODUCT_NUMBER,
    ...
    CAST(TRIM(W2MECH) AS NUMBER(10, 0)) AS MECHANIC_NUMBER,
    ...
    CAST(TRIM(W2CTCD) AS NUMBER(3, 0)) AS INVENTORY_CATEGORY_CODE,
...
source_with_keys AS (
    ...
    LEFT JOIN  {{ source('silver_data', 'T_DIM_PRODUCT') }} dp
        ON sd.PRODUCT_NUMBER = dp.ITEM_NUMBER AND dp.IS_CURRENT_FLAG = TRUE
    LEFT JOIN {{ source('silver_data', 'T_DIM_MECHANIC') }} mech
        ON sd.MECHANIC_NUMBER = mech.MECHANIC_ID AND mech.IS_CURRENT_FLAG = TRUE 
    ...
    LEFT JOIN {{ source('silver_data', 'T_DIM_INVENTORY_CATEGORY') }} ic
        ON sd.INVENTORY_CATEGORY_CODE = ic.CATEGORY_ID AND ic.IS_CURRENT_FLAG = TRUE
