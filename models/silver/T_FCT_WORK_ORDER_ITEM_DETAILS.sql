...
source_data AS (
 SELECT
    ...
    -- Join fields from dimension tables
    ...
    mech.MECHANIC_SK as MECHANIC_SK,
    dp.INVENTORY_CATEGORY_SK AS INVENTORY_CATEGORY_SK,
    ...
    -- Enhanced MD5 hash with join fields included
    MD5(CONCAT(
        COALESCE(TRIM(src.W2ITM), ''),
        COALESCE(TRIM(src.W2MFCD), ''),
        COALESCE(TRIM(src.W2QTY), ''),
        COALESCE(TRIM(src.W2SIZE), ''),
        COALESCE(TRIM(src.W2DESC), ''),
        COALESCE(TRIM(src.W2PLY), ''),
        COALESCE(TRIM(src.W2PRCD), ''),
        COALESCE(TRIM(src.W2UNCS), ''),
        COALESCE(TRIM(src.W2UNPR), ''),
        COALESCE(TRIM(src.W2AMT), ''),
        COALESCE(TRIM(src.W2CAMT), ''),
        COALESCE(TRIM(src.W2GPAM), ''),
        COALESCE(TRIM(src.W2GPMG), ''),
        COALESCE(TRIM(src.W2GPNM), ''),
        COALESCE(TRIM(src.W2GPNG), ''),
        COALESCE(TRIM(src.W2MECH), ''),
        COALESCE(TRIM(src.W2IEXM), ''),
        COALESCE(TRIM(src.W2ITXB), ''),
        COALESCE(TRIM(src.W2ISLT), ''),
        COALESCE(TRIM(src.W2IAMT), ''),
        COALESCE(TRIM(src.W2CSTS), ''),
        COALESCE(TRIM(src.W2RSTS), ''),
        COALESCE(TRIM(src.W2QITM), ''),
        COALESCE(TRIM(src.W2PITM), ''),
        COALESCE(TRIM(src.W2SFCD), ''),
        COALESCE(TRIM(src.W2ISAM), ''),
        COALESCE(TRIM(src.W2ISPT), ''),
        COALESCE(TRIM(src.W2IFEE), ''),
        COALESCE(TRIM(src.W2OUPR), ''),
        COALESCE(TRIM(src.W2QSTS), ''),
        COALESCE(TRIM(src.W2NSTS), ''),
        COALESCE(TRIM(src.W2VEND), ''),
        COALESCE(TRIM(src.W2VDNM), ''),
        COALESCE(TRIM(src.W2INVC), ''),
        COALESCE(TRIM(src.W2INDT), ''),
        COALESCE(TRIM(src.W2RSTK), ''),
        COALESCE(TRIM(src.W2CTCD), ''),
        COALESCE(TRIM(src.W2VSEQ), ''),
        COALESCE(TRIM(src.W2FDTX), ''),
        COALESCE(TRIM(src.W2AVCS), ''),
        COALESCE(TRIM(src.W2NAVD), ''),
        COALESCE(TRIM(src.W2NAPR), ''),
        COALESCE(TRIM(src.W2NADP), ''),
        COALESCE(TRIM(src.W2NADA), ''),
        COALESCE(TRIM(src.W2NAD2), ''),
        COALESCE(TRIM(src.W2NAD3), ''),
        COALESCE(TRIM(src.W2NAD4), ''),
        COALESCE(TRIM(src.W2NACD), ''),
        COALESCE(TRIM(src.W2NACR), ''),
        COALESCE(TRIM(src.W2NACP), ''),
        COALESCE(TRIM(src.W2CAMTGP), ''),
        COALESCE(TRIM(src.W2AMTGP), ''),
        COALESCE(TRIM(src.W2ACLN), ''),
        COALESCE(TRIM(src.W2GPDS), ''),
        -- Include join fields in hash
        COALESCE(dim.ORIGIN_TRANSACTION_SK::VARCHAR, ''),
        COALESCE(dim.WORK_ORDER_HEADER_SK::VARCHAR, ''),
        COALESCE(dp.PRODUCT_SK::VARCHAR, ''),
        COALESCE(mech.MECHANIC_SK::VARCHAR, ''),
        COALESCE(ic.INVENTORY_CATEGORY_SK::VARCHAR, '')
    ...
    -- Add joins to dimension tables in source CTE
    INNER JOIN {{ ref('T_FCT_WORK_ORDER_HEADER') }} dim
      ON dim.STORE_NUMBER = CAST(TRIM(src.W2STORE) AS NUMBER(3, 0))
      and dim.FORM_TYPE_CODE = CAST(TRIM(src.W2FMTP) AS VARCHAR(2))
      and dim.POS_PREFIX = CAST(TRIM(src.W2WIPX) AS VARCHAR(3))
      and dim.WORK_ORDER_NUMBER = CAST(TRIM(src.W2WO) AS NUMBER(10, 0))
     AND dim.IS_CURRENT_FLAG = TRUE
    LEFT JOIN  {{ source('silver_data', 'T_DIM_PRODUCT') }} dp
        ON CAST(TRIM(src.W2ITM) AS NUMBER(10, 0)) = dp.ITEM_NUMBER AND TO_TIMESTAMP_NTZ(TRIM(src.ENTRY_TIMESTAMP)) BETWEEN dp.EFFECTIVE_DATE AND COALESCE(dp.EXPIRATION_DATE, '9999-12-31')
    LEFT JOIN {{ source('silver_data', 'T_DIM_MECHANIC') }} mech
        ON CAST(TRIM(src.W2MECH) AS NUMBER(10, 0)) = mech.MECHANIC_ID AND TO_TIMESTAMP_NTZ(TRIM(src.ENTRY_TIMESTAMP)) BETWEEN mech.EFFECTIVE_DATE AND COALESCE(mech.EXPIRATION_DATE, '9999-12-31') 
    
    {% if is_incremental() %}
    WHERE src.ENTRY_TIMESTAMP ='1900-01-01T00:00:00Z'
        --WHERE src.ENTRY_TIMESTAMP > (SELECT COALESCE(MAX(EFFECTIVE_DATE), '1900-01-01') FROM {{ this }})
    {% endif %}
),

-- Step 2: Generate surrogate key - simplified since joins are already in source_data
source_with_keys AS (
    SELECT
        ROW_NUMBER() OVER (
