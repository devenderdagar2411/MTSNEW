...
new_rows AS (
    SELECT
        ...
        NULL::NUMBER(38,0) AS INVENTORY_CATEGORY_SK,
        oc.LAST_MAINTAINED_USER AS LAST_MODIFIED_USER,
        TO_TIMESTAMP_NTZ(oc.LAST_MAINTAINED_DATE) AS LAST_MODIFIED_DATE,
        oc.LAST_MAINTAINED_TIME AS LAST_MODIFIED_TIME,
        oc.LAST_MAINTAINED_WORKSTATION AS WORKSTATION_ID,
...
expired_rows AS (
    SELECT
        CAST(old.INVENTORY_CLASS_SK AS NUMBER(38,0)) AS INVENTORY_CLASS_SK,
        old.CLASS_ID,
        old.CLASS_NAME,
        old.CATEGORY_CODE,
        ...
        TO_TIMESTAMP_NTZ(old.LAST_MODIFIED_DATE) AS LAST_MODIFIED_DATE,
        old.LAST_MODIFIED_TIME,
        old.WORKSTATION_ID,
        TO_TIMESTAMP_NTZ(old.EFFECTIVE_DATE) AS EFFECTIVE_DATE,
        TO_TIMESTAMP_NTZ(new.EFFECTIVE_DATE - INTERVAL '1 SECOND') AS EXPIRATION_DATE,
...
soft_deleted_rows AS (
    SELECT
        ...
        old.WORKSTATION_ID,
        TO_TIMESTAMP_NTZ(old.EFFECTIVE_DATE) AS EFFECTIVE_DATE,
        TO_TIMESTAMP_NTZ(del.ENTRY_TIMESTAMP) AS EXPIRATION_DATE,
        FALSE AS IS_CURRENT_FLAG,
        old.SOURCE_SYSTEM,
        old.SOURCE_FILE_NAME,
        old.BATCH_ID,
