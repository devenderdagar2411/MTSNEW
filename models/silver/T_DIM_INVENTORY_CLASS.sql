...
WITH source_data AS (
    SELECT
        CAST(TRIM(C2CLCD) AS NUMBER(4,0))AS CLASS_ID,
         CAST(TRIM(C2NAME) AS VARCHAR(40)) AS CLASS_NAME,
        CAST(TRIM(C2CTCD) AS NUMBER(3,0))AS CATEGORY_CODE,
        CAST(TRIM(C2IVDN) AS NUMBER(3,0))AS INVENTORY_VENDOR_NUMBER,
    ...
    FROM {{ source('bronze_data', 'T_BRZ_INV_CLASS_INCLAS') }}
     {% if is_incremental() %}   
...
changes_with_category AS (
    ...
    ON c.CATEGORY_CODE = ic.CATEGORY_ID
     LEFT JOIN {{ ref('T_DIM_INVENTORY_VENDOR') }} dim
      ON dim.VENDOR_NUMBER = c.INVENTORY_VENDOR_NUMBER
     AND c.ENTRY_TIMESTAMP BETWEEN dim.EFFECTIVE_DATE AND COALESCE(dim.EXPIRATION_DATE, '9999-12-31')
),
